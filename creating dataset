...
ROI_right = 150
ROI_left = 350
classes=input("ENTER THE CLASS FOR GESTURE INPUT: ")
split=input("PRESS Train:1 Test:0")
if(split=="1"):
    sp="train"
    n=300
else:
...
while True:
    ...
    if num_frames < 60:
        ...
        cv2.putText(frame_copy, "Show Your Gesture" , (200, 400), cv2.FONT_HERSHEY_SIMPLEX, 1, (0,0,255), 2)
        ...
        if hand is not None:
            ...
            cv2.imshow("Thresholded Hand Image", thresholded)
            if num_imgs_taken <= n:
                print(num_imgs_taken)
                a=cv2.imwrite((loc +"/"+ str(num_imgs_taken) + '.jpg'), thresholded)
            else:
                break
            num_imgs_taken +=1
        else:
            cv2.putText(frame_copy, 'Show Gesture', (200, 400), cv2.FONT_HERSHEY_SIMPLEX, 1, (0,0,255), 2)
